...
| AI-based CAD optimization | Ingests a part's 3D CAD geometry and spatially checks it against corporate design norms and construction guidelines (wall thickness, tolerances, mounting-point standards, draft angles for castability). Flagged violations are paired with newly generated CAD proposals that resolve them while preserving the part's functional intent — in the same spirit as generative-design / topology-optimization tools. | Descriptive/Diagnostic (compliance check against norms) & Generative (new CAD proposals) | Geometric deep learning on 3D data (point-cloud or mesh networks, e.g. PointNet++/MeshCNN) to detect norm violations, combined with generative CAD / topology optimization (3D diffusion models or parametric generative design) to produce compliant redesigns |
| RfQ-to-BOM extraction assistant | Converting incoming RfQs (Request for Quotations) into a Bill of Materials is very time-intensive today. An AI system extracts all requirements from the RfQ documents, matches the extracted requirements to products in the catalog via an LLM, and generates the resulting BOM automatically. | Descriptive/Diagnostic (requirement extraction from unstructured RfQ text) & Generative (assembling the structured BOM); the product-matching step also has a Prescriptive flavor (recommending the best-fit product per requirement) | NLP/GenAI: LLM-based information extraction (Intelligent Document Processing if RfQs are scanned/PDF) combined with semantic matching (RAG / embedding similarity search) against the product master catalog, and an LLM to assemble matched line items into a structured BOM |
...
- *CAD optimization*: resolves your existing to-do ("where does CAD optimization fit in?") — moved into the **Knowledge Management** table above (§4), because it operationalizes engineering knowledge (norms/guidelines) rather than product quality on the workpiece itself. Working details weren't specified, so the description assumes a plausible architecture (geometric deep learning for compliance checking + generative CAD for redesign proposals) — verify against the actual solution once known.
